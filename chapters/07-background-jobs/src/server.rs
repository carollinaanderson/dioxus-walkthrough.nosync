//! `#[server]` functions for orders: bridge between Dioxus UI and Postgres
//! order store + graphile_worker queue. Bodies run server-side only; the
//! `#[get]`/`#[post]` macros generate client-side stubs for the WASM build.
