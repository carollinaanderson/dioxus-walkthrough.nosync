...
fn main() {
    ...
    // graphile_worker worker, and serve the app. `ClerkAuthLayer` verifies the
    // Clerk session cookie on every request so server functions can read the
    // caller's identity via `current_auth()`.
    #[cfg(feature = "server")]
    dioxus::serve(|| async {
