...
//! The whole app, for now: one component, no routing, no server. Both of
//! those show up in later chapters — this one is just "does Dioxus run?".
...
pub fn App() -> Element {
    rsx! {
        ...
        main { class: "wrap",
            h1 { "MyApp" }
            p { class: "sub", "Chapter 1: hello, Dioxus." }
            section { class: "card",
                h2 { "It's alive!" }
                p {
                    "This is a plain client-side Dioxus app — everything runs in the "
                    "browser, compiled to WebAssembly. There's no server involved yet; "
                    "that's chapter 2."
                }
            }
        }
...
// We'll reuse this same stylesheet for the whole tutorial. Most of these
// classes aren't used until later chapters (tables, pills, forms) — you
// don't need to understand every rule right now, just paste it in once.
pub const CSS: &str = r#"
